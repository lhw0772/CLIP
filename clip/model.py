...
def build_model_conv_proj(state_dict: dict, input_size):
    vit = "visual.proj" in state_dict
    ...
    model = CLIP_conv_proj(
        embed_dim,
        input_size, vision_layers, vision_width, vision_patch_size,
        context_length, vocab_size, transformer_width, transformer_heads, transformer_layers
    )
    ...
    for k, v in old_state_dict.items():
        if k in state_dict.keys():
            if 'visual' in k and ('v_proj' in k or 'c_proj' in k):
                # if 'weight' in k:
                #     # c_out, c_in = state_dict[k].shape
                #     old_state_dict[k] = state_dict[k][:, :, None, None]
                # else:
...
class ModifiedResNet_conv_proj(nn.Module):
        """
        A ResNet class that is similar to torchvision's but contains the following changes:
        - There are now 3 "stem" convolutions as opposed to 1, with an average pool instead of a max pool.
        - Performs anti-aliasing strided convolutions, where an avgpool is prepended to convolutions with stride > 1
        ...
        def __init__(self, layers, output_dim, heads, input_resolution=224, width=64):
            ...
            self.attnpool = AttentionConv(self.embed_dim, input_resolution // 32, output_dim)
...
class CLIP_conv_proj(nn.Module):
    def __init__(self,
                 embed_dim: int,
                 # vision
                 image_resolution: int,
        ...
        if isinstance(vision_layers, (tuple, list)):
            vision_heads = vision_width * 32 // 64
            self.visual = ModifiedResNet_conv_proj(
                layers=vision_layers,
                output_dim=embed_dim,
                heads=vision_heads,
                input_resolution=image_resolution,
